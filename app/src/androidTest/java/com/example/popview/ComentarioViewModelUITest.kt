    ...
    class ComentarioViewModelUITest {
        ...
        fun testRatingInvalid_showsError() {
            // 1) Escribe comentario válido
            onView(withId(R.id.editTextComment))
                .perform(typeText("Que maca"), closeSoftKeyboard())
            // 2) Cambia el rating programáticamente
            onView(withId(R.id.ratingBarComment)).perform(setRating(5f))
            // 3) Click enviar
            onView(withId(R.id.buttonSend)).perform(click())
            // 4) Espera
            Thread.sleep(500)
            // 5) Comprueba visibilidad y texto
            onView(withId(R.id.ratingErrorText))
